...
export function addHelpButton(scene: Phaser.Scene, lines: string[]) {
  ...
  helpBtn.on('pointerdown', () => {
    ...
    panel.fillRoundedRect(width / 2 - 180, height / 2 - 120, 360, 240, 12);
    panel.lineStyle(2, 0x6c5ce7, 0.5);
    panel.strokeRoundedRect(width / 2 - 180, height / 2 - 120, 360, 240, 12);

    const title = scene.add.text(width / 2, height / 2 - 100, 'Room Guide', {
      fontFamily: '"Inter"', fontSize: '14px', color: '#a29bfe', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(52);

    const body = scene.add.text(width / 2, height / 2 - 30, lines.join('\n'), {
      fontFamily: '"Inter"', fontSize: '13px', color: '#c8d6e5',
      wordWrap: { width: 300 }, lineSpacing: 8, align: 'center',
    }).setOrigin(0.5).setDepth(52);

    const closeIcn = scene.add.text(width / 2 + 160, height / 2 - 110, '✕', {
