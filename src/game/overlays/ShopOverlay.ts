...
export function openShop(scene: Phaser.Scene) {
  ...
  panel.fillRoundedRect(cx - 220, height / 2 - 180, 440, 360, 12);
  panel.lineStyle(2, 0xf39c12, 0.5);
  panel.strokeRoundedRect(cx - 220, height / 2 - 180, 440, 360, 12);

  const title = scene.add.text(cx, height / 2 - 158, "SAL'S SUPPLIES", {
    fontFamily: '"Press Start 2P"', fontSize: '11px', color: '#f39c12',
  }).setOrigin(0.5).setDepth(52);

  const coinDisp = scene.add.text(cx, height / 2 - 135, `Coins: ${gameStore.getState().playerData.coins}`, {
  ...
  for (const item of shopItems) {
    ...
    bg.fillRoundedRect(cx - 190, y, 380, 44, 6);
    bg.lineStyle(1, 0xf39c12, 0.2);
    bg.strokeRoundedRect(cx - 190, y, 380, 44, 6);

    const icon = scene.add.text(cx - 170, y + 10, item.symbol, {
      fontSize: '18px', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(53);

    const name = scene.add.text(cx - 140, y + 4, item.name, {
      fontFamily: '"Inter"', fontSize: '13px', color: item.color, fontStyle: 'bold',
    }).setDepth(53);

    const desc = scene.add.text(cx - 140, y + 24, item.description.substring(0, 30), {
      fontFamily: '"Inter"', fontSize: '10px', color: '#7a6a4a',
    }).setDepth(53);

    const btnG = scene.add.graphics().setDepth(53);
    btnG.fillStyle(0xf39c12, 0.85);
    btnG.fillRoundedRect(cx + 100, y + 5, 80, 26, 6);

    const btnT = scene.add.text(cx + 140, y + 18, `${item.price}`, {
      fontFamily: '"Inter"', fontSize: '11px', color: '#fff', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(54);

    const zone = scene.add.zone(cx + 140, y + 18, 80, 26)
  ...
  const closeBtn = scene.add.text(cx + 200, height / 2 - 170, '✕', {
